...
src/
  ...
  types.ts                 // 5절의 타입 정의
  data/
    seoulDongs.ts          // 서울 25개 구 행정동과 대표 좌표
...
이번 단계에서는 목 데이터를 반환한다. 기본 호출에는 300ms 지연을 넣어 비동기 흐름을 실제와 동일하게 만들고, 시연 컨트롤에서 시나리오를 명시하면 즉시 반환한다.
...
| 질문 | 선택지 |
|---|---|
| 어디 사세요 | 서울 25개 자치구의 행정동 검색 |
| 몇 층인가요 | 반지하 — 창문이 지면보다 낮아요 / 1층 — 지면과 같은 높이예요 / 중간층 — 위아래 모두 집이에요 / 최상층 — 바로 위가 옥상이에요 |
...
지역은 동 이름이나 구 이름으로 실시간 검색한다. 입력 전에는 `동 이름을 입력하세요`를 표시하고, 검색 결과는 `서교동 · 마포구` 형식으로 최대 8개까지 보여준다. 결과가 없으면 `검색 결과가 없어요`를 표시하며, 위·아래 방향키와 Enter 키로도 결과를 선택할 수 있어야 한다. 선택한 행정동의 위·경도는 `latLonToGrid`로 기상청 격자 좌표로 변환하고, 주소는 `{ label, nx, ny }` 형태로 저장한다. `label`은 `서울 마포구 서교동` 형식을 사용한다.

층수 버튼은 제목 아래에 위 설명을 한 단계 작은 흐린 글씨로 표시하며, 네 버튼의 높이와 내부 정렬을 동일하게 유지한다.

메인 헤더의 현재 지역명은 방 정보 설정을 여는 버튼으로 사용한다. 밑줄이나 작은 아이콘으로 누를 수 있음을 표시한다.

- 처음 진입할 때는 선택값 없이 시작하고 `선택 완료` 버튼만 표시한다.
- 설정을 다시 열 때는 저장된 지역·층수·방향·창호를 미리 선택하고 완료 버튼 문구를 `저장`으로 바꾼다.
- 수정 모드에는 `취소` 버튼을 표시한다. 취소하면 저장값을 변경하지 않고 기존 메인 화면으로 돌아간다.
- 저장하면 변경된 방 정보를 localStorage에 기록하고 새 조건으로 메인 화면을 다시 계산한다.
...
위험도 추이 그래프 바로 아래에 항상 표시하는 `내 방 조건 바꿔보기` 인라인 섹션. 층수·방향·창호를 한 항목씩 가로 버튼으로 배치하며, 값을 바꾸면 위험도가 **즉시** 다시 계산되어 게이지와 그래프에 반영된다. 저장되지 않는 미리보기임을 작은 글씨로 알리고, `원래대로` 버튼을 누르면 저장된 방 조건으로 돌아간다.
...
온보딩 주소는 `seoulDongs.ts`의 서울 전역 행정동을 사용한다. 서교동 `(59, 127)`, 신림동 `(59, 125)`, 안암동 `(60, 128)` 기준 좌표는 목 시나리오 시연용 매핑으로만 유지한다.

`fetchWeather(nx, ny)`처럼 시나리오를 생략한 기본 호출에서는 좌표별로 서로 다른 목 날씨를 반환한다. 서교동 `(59, 127)`은 `rainy`, 신림동 `(59, 125)`은 `mild`, 안암동 `(60, 128)`은 `mild`에 기온 +2℃와 습도 +12%를 적용한다. 목록에 없는 좌표는 `mild`를 기본값으로 사용하고 좌표를 지역 라벨에 표시한다. 난수는 사용하지 않으며 같은 좌표는 항상 같은 결과를 반환한다. `?demo=1`에서 시나리오를 명시한 호출은 지역 변형 없이 해당 시나리오 원본을 그대로 사용한다.
